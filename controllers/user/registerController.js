...
const handleNewUser = async (req, res) => {
  const { username, password, matchedPassword, email, firstname, lastname } = req.body;
  if (!username || !password || !email) {
    ...
    if (!matchedPassword) missingFields.push('matched password');
  ...
  try {
    const duplicate = await User.findOne({ $or: [{ username: username }, { email: email }] }).exec();
    if (duplicate) {
    ...
    console.log(result);
    res.status(201).json({ success: `New user ${username} created. Check your email for verification.` });
  } catch (err) {
